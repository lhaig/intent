...
### Phase 42: Stage2 Formatter CLI Wiring + Differential Test -- IN PROGRESS (2026-06-15)
Turns the stage2 formatter from a test-only library into a runnable, CLI-wired tool measured against stage1 on real code. CLI-wiring half complete; parser-gap closing ongoing.
- **42.1 `args()` builtin** ([ADR 0045](decisions/0045-args-builtin.md)) — the command-line-argument primitive self-hosted tools need. `args() -> Array<String>`, program/script name at index 0 (Rust+Go convention). Rust `std::env::args().collect()`, JS `process.argv.slice(1)` (normalized so the script is index 0), WASM stub. Three-layer plumbing mirroring `timestamp_ms`.
- **42.2 `main.intent`** — the stage2 formatter as a runnable program: reads `args()[1]`, parses, prints `format_program(...)` (exit 0/1/2/3). Built + run byte-equal with the source on hello.intent (rust + js). Surfaced and fixed a latent stage1 multi-file bug: an `entry` function in an *imported* module emitted a duplicate `main` — gated the entry wrapper on `f.IsEntry && g.isEntryFile` so imported entry functions demote to ordinary functions (rustbe + jsbe; +2 regression tests).
- **42.3 differential harness** — `selfhost/formatter/difftest.sh` + `make diff-formatter`. Canonicalize-first (stage1-`fmt` a copy, then check stage2 reproduces it byte-for-byte) = a true comparison against `intentc fmt`. Per-file PASS/DIVERGE/PARSE-ERR table; gate exit code. **Key result: zero true divergences** — whenever the stage2 parser accepts a file, the formatter matches stage1 exactly, so all remaining work is parser coverage.
- **42.4 `intentc fmt --self-hosted`** — Go shim delegating to the stage2 binary (env `INTENT_STAGE2_FMT` override or auto-build-with-cache). Composes with `--check`; a stage2 parse error exits non-zero with no stage1 fallback. Exec layer unit-tested with fake binaries (no cargo); end-to-end byte-equal with native fmt.
- **42.5 invariants (+ constructor contracts + intent blocks)** — first harness-driven gap close. Fixed the wrong `invariant { }` block form to `invariant <expr>;` clauses positioned between fields and constructor (matching stage1); folded in constructor contract clauses and the real `intent "desc" { ...; verified_by: [...] }` surface form (needed by the target files).

Corpus status: **16/22 examples agree with `intentc fmt`**, 0 divergences. Remaining parser gaps (each a small parse+format add): `forall`/`exists`, `implies`, generic decl params `<T>`, `Fn(...)`/lambdas, `async`/`await`, attributes `@name(...)`. PRD: `prds/active/phase-42-formatter-cli-differential.md`.
