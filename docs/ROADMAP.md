...
Strategic frame in [ADR 0040](decisions/0040-self-hosted-formatter-strategy.md): build a stage2 Intent formatter (`selfhost/formatter/`) that parses and re-emits Intent source, eventually replacing the Go formatter at byte parity. Stage1 / stage2 mental model adapted from Zig precedent. Delivered gap-driven across multiple phases — each phase first lands the language extension it needs in stage1 (Go), then writes stage2 Intent code that uses it. First gap: string indexing + `Char` type ([ADR 0041](decisions/0041-string-indexing-and-char-type.md), `ops/plans/phase-31-string-primitives.md`). Phase 32 lexer, 33-36 parser, 37 formatter, 38 full-feature parity, 39 differential test gate + CLI integration. Phase numbers indicative; will shift as gaps surface. Per-phase ADRs follow as needed (sum types, dynamic dispatch, I/O streaming, etc.).
