<aiki version="0.2.8" hash="dcdc56e79fc2500b">
...
<!-- Everything below is OUTSIDE the aiki-managed block above; aiki will not modify it. -->

# About Intent

Intent is a contract-first programming language **designed for AI code assistants to write**,
compiling to multiple targets for humans to run — native binaries (via Rust), JavaScript, and
WebAssembly — from a single source file through an internal, target-independent IR. The
compiler toolchain is written in Go.

**Aims:**

- **Correct, auditable AI-generated code.** Every function carries `requires`/`ensures`
  contracts, every entity carries invariants, and `intent` blocks link natural-language goals
  to formal `verified_by` points — the language favours explicit contracts and declared intent
  over brevity.
- **Verifiable correctness.** Contracts compile to runtime assertions in each target's idiom
  and can be proven statically with Z3 (`intentc verify`).
- **A self-hosting toolchain.** The formatter, linter, checker, and compiler are each written
  in Intent and reproduce the Go implementation byte-for-byte, proven by a
  `stage1 → stage2 → stage3` bootstrap fixpoint (`make bootstrap-stage3`).

# Where to find things

| Document | What's in it |
| --- | --- |
| `README.md` | Project overview, prerequisites, and the build / test / run commands. |
| `INTENT.md` | How to write Intent programs — the AI code-generation guide. |
| `docs/DESIGN.md` | The full language specification. |
| `docs/grammar.ebnf` | The formal grammar. |
| `docs/decisions/` | Architecture Decision Records — the "why" behind design choices (start with `0000-why-intent-exists`). |
| `docs/ROADMAP.md` | Milestones and current status. |
| `docs/HARNESS.md` | The validation/test harness — `make` targets and the gate suite. |

**Conventions:** run `make build`, `make test`, and `make validate` before claiming a change is
done (full table in `docs/HARNESS.md`); record non-obvious design choices as ADRs in
`docs/decisions/`. Commits are conventional (`feat:` / `fix:` / `docs:` / …), with no Claude
co-author and no emojis.
