...
| 48j-c2 | remaining builtin arg typing + await_* async-context | — | TODO | assert_eq (`assert_eq() type mismatch: actual is X, expected is Y` + Equal/comparable-set + entity eq method), len (`len() requires Array, Map, or String argument, got X`, .String() of generics), assert_panics (Fn()->Void). await_all/await_any/timeout need async-context tracking (`await can only be used inside async functions`) which stage2's checker lacks — that inAsyncFunc flag is the real unlock. Plus unary operator-typing (needs ex_unary positions) |
