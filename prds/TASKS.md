...
| 48j-a | match-arm consistency/exhaustiveness | — | TODO | stage1 checkMatchExpr (checker.go:2915): `duplicate match arm for variant 'V'`, `match arm type mismatch: expected X, got Y`, `non-exhaustive match on enum 'E': missing variants: ...`. Needs match-expr inference (arm-body types + enum-variant coverage). Larger; likely multiple slices |
