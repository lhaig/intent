...
| BE-1 | rustbe: auto-borrow call-result args to `Array<T>` (&Vec) params | — | TODO | Discovered in 46.3. `f(make_vec())` where `f(x: Array<T>)` emits `f(make_vec())` not `f(&make_vec())` → E0308 (expected &Vec, found Vec). Only lvalue args are auto-borrowed. Workaround: bind to a `let` first. JS unaffected. Latent self-hosting blocker |
| HARN-1 | test runner: surface swallowed rust cargo-compile errors | — | TODO | Discovered in 46.3. runRustTests: when the crate fails to compile, parseCargoTestOutput returns one "did not run (build or harness failure)" row PER declared test, so `len(results)==0` is false and the raw `cargo test` stderr (the real E-code + location) is never surfaced. Had to patch/keep the tmpdir to see it. Fix: detect all-missing and return the cargo error |
