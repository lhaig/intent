...
| 48j-c | builtin argument typing (uniform-type group + print) | ADR 0056 | DONE (2026-07-03) | builtin_arg_type table: assert(Bool), char_from_codepoint/sleep(Int), read_file/write_file/create_dir/file_exists/env_get/http_post/http_get/json_get/json_path/emit_event(String) → `NAME() argument [N ]must be T, got X` (numbered iff arity>1) at the call; print → `print() cannot print type X (accepts Int, Float, Bool, String)` (Char rejected; uses .name). Confident + no-type_args args only. commits 42b3f0c + bd3d22f. +4 fixtures (diff-checker 74/74), +7 tests (253) |
| 48j-c2 | remaining builtin arg typing + await_* async-context | — | TODO | assert_close (3 labeled Float args: `assert_close() argument N (label) must be Float, got X`), assert_eq (Equal + comparable set, entity eq method), len (Array/Map/String, .String() of generics), assert_panics (Fn()->Void). await_all/await_any/timeout need async-context tracking (`await can only be used inside async functions`) which stage2's checker lacks. Plus unary operator-typing (needs ex_unary positions) |
