...
| 48j-a2 | match arm-type consistency + scrutinee-must-be-enum | — | TODO | `match arm type mismatch: expected X, got Y` (needs arm-body inference — type each arm body, compare to the first) and `match scrutinee must be an enum type, got X` (needs enum-vs-nonenum certainty — Option/Result must not be flagged). Deferred from 48j-a |
| 48j-c | builtin argument typing + await_* async-context | — | TODO | deferred from Phase 47 (hang off the builtin table). Plus unary operator-typing (needs ex_unary positions) |
