...
| 46.4b | `unknown type 'X'` — remaining sites | [prd-phase-46-checker-type-foundation.md](active/prd-phase-46-checker-type-foundation.md) | TODO | Deferred from 46.4a: (1) entity FIELD types — needs FieldDecl line/column (a small additive front-end change, 45.7 precedent; ADR 0053 D1 was about type-representation, not positions); (2) entity method + constructor params/returns (interleave into check_dup_entities using entity.type_params; verify ctor is in methods); (3) enum-variant field types (registerEnums, BEFORE entities registered — quirk; verify variant Param positions, parser:830 creates some Params w/o line/col); (4) `let` stmt types (check-phase, thread enclosing decl type_params into check_body_stmts); (5) externs (ExternDecl) if parsed. Match stage1 emit order each |
