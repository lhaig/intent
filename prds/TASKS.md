...
| 48j-c2b | assert_panics() argument typing | ADR 0056 | DONE (2026-07-03) | assert_panics() requires `Fn() -> Void` (stage1 checker.go:1728). A Fn type always carries type_args (>= its return), so a confidently-inferred no-type_args type is definitely a non-function that stage1 rejects → `assert_panics() argument must be Fn() -> Void, got X` at the call. Fn-typed params + lambdas skip soundly (lambda infers Unknown; a wrong-shape `Fn(Int)->Void` param has type_args → deferred false negative needing Fn .String()). commit TBD. +1 fixture (diff-checker 77/77), +2 tests (260) |
| 48j-c2 | remaining builtin arg typing + await_* async-context | — | TODO | assert_eq (`assert_eq() type mismatch: actual is X, expected is Y` + Equal/comparable-set + entity eq method). await_all/await_any/timeout need async-context tracking (`await can only be used inside async functions`) which stage2's checker lacks — that inAsyncFunc flag is the real unlock. Plus unary operator-typing (needs ex_unary positions) |
