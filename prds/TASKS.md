...
| 48g | variant-constructor argument-type mismatch | — | TODO | Analogous to 48f but for variant calls; message `variant 'V' field 'f' expects X, got Y` (checker.go ~388); look up variant field types from prog.enums. Reuses prog + infer |
| 48h | assignment-stmt type-mismatch | — | TODO | `x = expr` where inferred RHS != x's type (st_assign; needs the target's type from scope) |
| 48i | method-call arity + arg-types | — | TODO | Needs the receiver's entity type → `self` typed as the entity + field-access inference (self.field → field type). Larger |
