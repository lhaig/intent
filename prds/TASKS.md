...
| 46.4b.3 | enum-variant fields, let-stmts, externs | [prd-phase-46-checker-type-foundation.md](active/prd-phase-46-checker-type-foundation.md) | TODO | Remaining: (a) enum-variant field types (registerEnums, runs BEFORE entities registered → resolves against EMPTY entity set, a quirk to match; verify variant Param positions — parser:830 builds some Params w/o line/col); (b) `let` stmt types (check-phase, thread enclosing decl type_params into check_body_stmts); (c) externs (ExternDecl) if the stage2 parser emits them. Match stage1 emit order each |
