...
| 45.3 | Structural checks: dup enum variant + return-in-test | [prd-phase-45-self-hosted-checker.md](active/prd-phase-45-self-hosted-checker.md) | TODO | `duplicate variant name`, `'return' not allowed in test body` (%q-quoted name); no symbol table; needs: 45.2 |
| 45.4 | Stage2 break/continue statement support | [prd-phase-45-self-hosted-checker.md](active/prd-phase-45-self-hosted-checker.md) | TODO | DISCOVERED PREREQ: stage2 lexes break/continue as identifiers (no kw); add kw_break/kw_continue + st_break/st_continue + parser + formatter emit. Unblocks the outside-loop check AND prevents undeclared-var false-positives on error_handling.intent; needs: 45.2 |
